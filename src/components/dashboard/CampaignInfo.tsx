    ...
    <div className="space-y-10">
      ...
      >
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <h3 className="font-display font-black text-foreground uppercase tracking-[0.3em] text-xs">Sứ mệnh chiến dịch</h3>
        </div>
      ...
      >
        ...
        <ul className="space-y-6">
          {[
            { text: "Môn thể thao: Chạy bộ (Run)", icon: ActivityIcon, desc: "Tất cả các hoạt động phải thuộc phân loại Run trên Strava." },
            { text: "Pace hợp lệ: 4:00 - 15:00 /km", icon: MapPin, desc: "Đảm bảo pace nằm trong giới hạn để được tính điểm." },
            { text: "Khoảng cách tối thiểu: 1.0 km", icon: MapPin, desc: "Mỗi hoạt động cần đạt ít nhất 1km để được đồng bộ." }
          ].map((rule, i) => (
            <li key={i} className="flex items-start gap-6 group hover:translate-x-2 transition-transform duration-300">
              ...
              <div className="flex-1">
                <div className="text-[15px] font-black text-foreground tracking-tight uppercase italic">{rule.text}</div>
                <p className="text-[13px] text-muted-foreground font-bold mt-1 leading-snug">{rule.desc}</p>
              </div>
            </li>
          ))}
        </ul>
