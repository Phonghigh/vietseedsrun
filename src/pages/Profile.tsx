    ...
    <AppLayout>
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <div className="grid lg:grid-cols-2 gap-6">
          ...
          >
            ...
            {trendData.length > 0 ? (
              <ResponsiveContainer width="100%" height={180}>
                ...
                <div className="h-[180px] flex items-center justify-center border border-dashed border-border rounded-lg text-muted-foreground italic text-sm">
          ...
          >
            <h2 className="font-display text-lg font-semibold text-foreground mb-4 flex items-center gap-2">
               <Trophy className="h-5 w-5 text-primary" /> Thống kê cá nhân
            </h2>
            <div className="grid grid-cols-2 gap-4">
              <div className="p-4 bg-primary/5 rounded-xl border border-primary/10">
                <div className="text-xs text-muted-foreground uppercase tracking-wider mb-1">Tổng dặm bay</div>
                <div className="text-2xl font-bold text-foreground">{(user?.totalDistance || 0).toLocaleString()} <span className="text-sm font-normal">km</span></div>
              </div>
              <div className="p-4 bg-accent/5 rounded-xl border border-accent/10">
                <div className="text-xs text-muted-foreground uppercase tracking-wider mb-1">Hoạt động</div>
                <div className="text-2xl font-bold text-foreground">{user?.totalActivities || 0}</div>
              </div>
            </div>
            <p className="mt-6 text-xs text-muted-foreground leading-relaxed">
                Tất cả dữ liệu được đồng bộ trực tiếp từ tài khoản Strava của bạn. Hoạt động cần đáp ứng tiêu chuẩn về Pace (4 - 15 min/km) để được tính vào tổng thành tích.
            </p>
          </motion.div>
        </div>
